...
class Inference(object):
    ...
    def create_model(self):
        ...
        if self.model not in ['chatgpt', 'gpt4']:
            if self.model.lower() =='llama2-13b-chat':
                # Use 8 bit quantized version of Llama 2 13B Chat
                quantization_config = BitsAndBytesConfig(
                    load_in_8bit=True,
                )
                self.tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-13b-chat-hf", device_map="auto")
                self.tokenizer.pad_token = '[PAD]'
                self.tokenizer.padding_side = 'left'
                self.pipe = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-13b-chat-hf", device_map="auto", torch_dtype=torch.float16, quantization_config=quantization_config)
            elif self.model.lower() =='llama2-7b-chat':
                # Llama 2 7B Chat
                self.tokenizer = LlamaTokenizer.from_pretrained("meta-llama/Llama-2-7b-chat-hf", device_map="auto")
                self.tokenizer.pad_token = '[PAD]'
                self.tokenizer.padding_side = 'left'
                self.pipe = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-chat-hf", device_map="auto", torch_dtype=torch.float16)               
            elif self.model.lower() =='llama3.1-8b-instruct':
                # Use Llama 3.1 8B Instruct
                self.tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct", device_map="auto", add_eos_token=True)
                # self.tokenizer.pad_token = '[PAD]'
                self.tokenizer.pad_token = self.tokenizer.pad_token or self.tokenizer.eos_token
                self.tokenizer.padding_side = 'left'
                self.pipe = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.1-8B-Instruct" , device_map="auto", torch_dtype=torch.float16)
    ...
    def process_input(self, prompt, raw_data,fs_num=None,gpt=False):
        # process inputs based on dataset type
    ...
    def predict_by_openai_api(self, model, prompt, debias_prompt):
        ...
        print(f"Score on {self.args.dataset}: {score}")
        return score

    def predict_by_local_inference(self, model, prompt, debias_prompt, fs_num=None):
        # predictions on inference of local data
        data_len = len(self.args.data)
    ...
    def call_openai_api(self, model, prompt):
        ...
        if model in ['chatgpt']:
            response = openai.ChatCompletion.create(model="gpt-3.5-turbo-1106",messages=[{"role": "user", "content": prompt}],temperature=0.001,top_p=0)
            result = response['choices'][0]['message']['content']
        else:
            response = openai.ChatCompletion.create(model='gpt-4-1106-preview',messages=[{"role": "user", "content": prompt}],temperature=0.001,top_p=0)
            result = response['choices'][0]['message']['content']
        return result
    ...
    def pred_by_generation(self, inputs, model):
        # generate outputs
        with torch.no_grad():
            outputs = self.pipe.generate(**inputs,temperature=0.001,early_stopping=True,return_dict_in_generate=True,output_scores=True,output_hidden_states=True)
        return outputs
